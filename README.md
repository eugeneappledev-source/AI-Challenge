...
Нативное iOS-приложение открывается каталогом заданий. День 1 демонстрирует базовый запрос к LLM, День 2 сравнивает свободный ответ с контролируемым JSON, День 3 исследует четыре способа рассуждения, а День 4 сравнивает ответы при `temperature = 0`, `0.7` и `1.2`. Для последних двух экспериментов предусмотрена отдельная AI-рецензия. Адаптивная web-версия сохраняет сценарий Дня 2 с пищевым ассистентом.
