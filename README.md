...
Нативное iOS-приложение работает как пищевой AI-ассистент и отправляет один запрос в собственный Go backend в двух режимах. Сервер получает свободный и контролируемый ответы DeepSeek, а SwiftUI-интерфейс позволяет сравнить их с помощью сегмента. Вопросы вне темы еды получают вежливый отказ.
