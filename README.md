...
Репозиторий построен как один развивающийся продукт, поэтому проверяющему не нужно искать пять отдельных проектов:
...
Нативное iOS-приложение открывается каталогом заданий. День 1 демонстрирует базовый запрос к LLM, День 2 сравнивает свободный ответ с контролируемым JSON, День 3 исследует четыре способа рассуждения, День 4 — три значения `temperature`, а День 5 сравнивает реальные версии моделей по качеству, задержке, токенам и расчётной стоимости. Для исследовательских сценариев предусмотрена отдельная AI-рецензия. Адаптивная web-версия сохраняет сценарий Дня 2 с пищевым ассистентом.
