...
Нативное iOS-приложение открывается каталогом заданий. Первая неделя исследует базовый API-вызов, управляемый JSON, способы рассуждения, `temperature` и версии моделей. Вторая неделя строит самостоятельного агента Compass, добавляет ему постоянную SQLite-память, прозрачные token/cost-метрики и управляемый контекст `summary + recent` с A/B-рецензией. Адаптивная web-версия сохраняет сценарий Дня 2 с пищевым ассистентом.
